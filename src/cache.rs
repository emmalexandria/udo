...
use crate::{backend::ProcessManager, config::Config, run::Run};
...
pub fn create_cache_dir(user: &User, backend: &dyn ProcessManager) -> Result<PathBuf> {
...
pub fn write_entry(user: &User, entry: CacheEntry, backend: &dyn ProcessManager) -> Result<()> {
...
pub fn clear_cache(user: &User, backend: &dyn ProcessManager) -> Result<()> {
    let dir = get_cache_dir(user);
